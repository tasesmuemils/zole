...
export default function GameData() {
	...
	return (
		<motion.main
			...
			// bg-white border-slate-100 transition-all duration-500 demo-dark:bg-slate-800 transition-all duration-500 demo-dark:border-slate-500 border-b rounded-t-xl p-4 pb-6 sm:p-10 sm:pb-8 lg:p-6 xl:p-10 xl:pb-8 space-y-6 sm:space-y-8 lg:space-y-6 xl:space-y-8
			ref={scrollRef}
		>
			<div>
				<table className="w-full text-sm text-center mb-20 shadow-xl text-gray-500 dark:text-gray-400">
					...
					{playersList[0].score.length > 0 && (
						<ScoreRows rowsCount={playersList[0].score.length} playersList={playersList} />
					)}
				</table>
				<Controls getScore={getScore} scores={scoreTable} players={playersList} />
			</div>
		</motion.main>
	);
}
	...
	for (let i = 0; i < rowsCount; i++) {
		arr.push(
			<motion.tr
				initial={{ opacity: 0 }}
				animate={{ opacity: 1 }}
				transition={{ duration: 0.3, delay: `${i == rowsCount - 1 ? 0.2 : 0.2}` }}
			...
			>
				...
				{playersList.length != 3 && <Customtd i={i} score={playersList[3].score[i]} rowsCount={rowsCount} />}
			</motion.tr>
		);
	}
...
const Customtd = ({ i, score, rowsCount }) => {
	return (
		<td
			className={`px-6 py-4 ${
				i == rowsCount - 1 && " font-extrabold transition-all duration-500 demo-dark:text-slate-100"
			}`}
		>
			<span className={`${i == rowsCount - 1 && "text-cyan-500 px-2 py-1 border-b-2 border-cyan-500"}`}>
				{score}
			</span>
		</td>
	);
