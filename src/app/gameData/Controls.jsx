...
export default function Controls({ scores, players, getScore }) {
		...
		const arr = players.map((player) => {
			if (player.player == winner) {
				player.score.length > 0
					? player.score.push(player.score[player.score.length - 1] + value["Lielajam"])
					: player.score.push(value["Lielajam"]);
			} else {
				player.score.length > 0
					? player.score.push(player.score[player.score.length - 1] + value["1.mazajam"])
	...
	return (
		<div
			ref={ref}
			className="z-0 bg-slate-50 text-gray-500 transition-all duration-500 dark:bg-slate-600 transition-all duration-500 dark:text-slate-200 w-full px-10 py-5 fixed bottom-0 left-0 flex justify-center"
		>
						...
						<div className="flex basis-1 flex-wrap justify-center">
							{Object.keys(scores[gameType]).map((scenario, index) => (
								<ControlsButton
									...
									text={scenario}
									index={index}
									onClickFunction={() => handleScenario(scores[gameType][scenario][players.length])}
								/>
							))}
						</div>
					</motion.div>
...
const ControlsButton = ({ text, index, onClickFunction }) => {
	return (
		<button
			className="rounded-lg text-sm leading-6 font-semibold px-3 m-2 ring-2 ring-inset hover:bg-cyan-500 dark:hover:bg-cyan-500 hover:ring-cyan-500 hover:text-slate-50 ring-slate-500 text-slate-500 transition-all duration-500 dark:text-slate-100 transition-all duration-500 dark:inset transition-all duration-500 dark:bg-slate-500"
